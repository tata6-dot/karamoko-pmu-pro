...
const PMUModals = {
  ...
  afficher(config) {
    ...
    overlay.innerHTML = `
      <div style="background:#16213e;padding:30px;border-radius:12px;max-width:500px;width:90%;border:1px solid #2a2a4a;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
          ...
          <button data-modal-close style="background:none;border:none;color:#a0a0a0;font-size:1.5rem;cursor:pointer;">×</button>
        </div>
        <div style="margin-bottom:20px;">${config.contenu}</div>
        <div style="display:flex;gap:10px;justify-content:flex-end;" id="modal-boutons"></div>
      </div>
    `;
    ...
    config.boutons.forEach(b => {
      ...
      btn.style.cssText = `padding:10px 20px;border:none;border-radius:8px;cursor:pointer;font-size:1rem;${b.classe === 'btn-primary' ? 'background:#e94560;color:white;' : b.classe === 'btn-danger' ? 'background:#f44336;color:white;' : 'background:#0f3460;color:white;'}`;
